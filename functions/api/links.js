...
// Route: /api/links
// Storage: Cloudflare KV binding named LINKS_KV
// Auth: simple shared-password header, checked against env.ADMIN_PASSWORD
...
// GET /api/links -> public, returns current link list
export async function onRequestGet({ env }) {
  const links = await getLinks(env);
  return json({ ok: true, links });
}

// POST /api/links -> { password, action, payload }
export async function onRequestPost({ request, env }) {
  ...
  switch (action) {
    ...
    case "reorder": {
      // payload.order = array of ids in the new order
